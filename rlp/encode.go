...
//首先定义了空字符串和空List的值，分别是 0x80和0xC0。 注意整形的0值的对应值也是0x80。
var (
...
func Encode(w io.Writer, val interface{}) error {
	if outer, ok := w.(*encbuf); ok {			//首先获取了一个encbuf对象。
	...
	eb.reset()
	if err := eb.encode(val); err != nil {		//调用这个对象的encode方法
		return err
	}
...
type encbuf struct {
	str     []byte      // string data, contains everything except list headers str字段包含了所有的内容，除了列表的头部
	lheads  []*listhead // all list headers                                     列表的头部记录
	lhsize  int         // sum of sizes of all encoded list headers             记录了lheads的长度
	sizebuf []byte      // 9-byte auxiliary buffer for uint encoding            9个字节大小的辅助buffer，专门用来处理uint的编码的
}

type listhead struct {
	offset int // index of this header in string data                  记录了列表数据在str字段的哪个位置
	size   int // total size of encoded data (including list headers)  记录了包含列表头的编码后的数据的总长度
}
...
func (w *encbuf) encode(val interface{}) error {
	rval := reflect.ValueOf(val)	//首先获取了对象的反射类型，根据反射类型获取它的编码器，然后调用编码器的writer方法。
...
func (w *encbuf) toBytes() []byte {		//会对listhead进行处理，组装成完整的RLP数据
