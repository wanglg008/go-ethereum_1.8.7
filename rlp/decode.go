...
func (s *Stream) Bytes() ([]byte, error) {
	...
	//通过s.Kind()方法获取当前需要解析的类型和长度，如果是Byte类型，那么直接返回Byte的值，如果是String类型那么读取指定长度的值然后返回。
	switch kind {
	case Byte:
		s.kind = -1 // rearm Kind
...
func (s *Stream) List() (size uint64, err error) {
	kind, size, err := s.Kind()	//调用Kind方法获取类型和长度，如果类型不匹配那么就抛出错误，然后把一个listpos对象压入到堆栈
	if err != nil {
		return 0, err
	}
	if kind != List {
	...
	//pos字段记录了当前这个list已经读取了多少字节的数据，所以刚开始的时候肯定是0. size字段记录了这个list对象一共需要读取多少字节数据。
	//这样在处理后续的每一个字段的时候，每读取一些字节，就会增加pos这个字段的值，处理到最后会对比pos字段和size字段是否相等，如果不相等，
	//那么会抛出异常。
...
func (s *Stream) ListEnd() error {
	//如果当前读取的数据数量pos不等于声明的数据长度size，抛出异常，然后对堆栈进行pop操作，如果当前堆栈不为空，
	//那么就在堆栈的栈顶的pos加上当前处理完毕的数据长度
	if len(s.stack) == 0 {
		return errNotInList
	}
