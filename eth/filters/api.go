...
func (api *PublicFilterAPI) timeoutLoop() {//每隔5分钟检查一下。 如果过期的过滤器，删除。
...
//用来创建一个PendingTransactionFilter。 这种方式是用来给那种无法创建长连接的通道使用的(比如HTTP), 如果对于可以建立长链接的通道(比如WebSocket)可以使用rpc提供的发送订阅模式来处理，就不用持续的轮询了
func (api *PublicFilterAPI) NewPendingTransactionFilter() rpc.ID {
	var (
		pendingTxs   = make(chan common.Hash)
		pendingTxSub = api.events.SubscribePendingTxEvents(pendingTxs)// 在事件系统订阅这种消息
	)
...
type FilterCriteria struct {
	FromBlock *big.Int             //开始的区块号
	ToBlock   *big.Int             //结束的区块号
	Addresses []common.Address     //根据合约 Addresses指定的地址过滤
	Topics    [][]common.Hash      //根据指定的Topics来过滤
}
...
func (api *PublicFilterAPI) GetLogs(ctx context.Context, crit FilterCriteria) ([]*types.Log, error) {
	...
	// Create and run the filter to get all the logs  // 创建了一个Filter对象 然后调用filter.Logs
...
// GetFilterChanges returns the logs for the filter with the given id since  GetFilterChanges用来返回从上次调用到现在的所有的指定id的所有过滤信息。这个可以用来轮询。
// last time it was called. This can be used for polling.
//
// For pending transaction and block filters the result is []common.Hash.    对于pending transaction和block的过滤器，返回结果类型是[]common.Hash. 对于pending Log 过滤器，返回的是 []Log
...
func (api *PublicFilterAPI) GetFilterChanges(id rpc.ID) (interface{}, error) {
	...
	if f, found := api.filters[id]; found {
		...
		f.deadline.Reset(deadline)// 如果定时器已经触发，但是filter还没有移除，那么我们先接收定时器的值，然后重置定时器
