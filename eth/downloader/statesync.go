...
// on its behalf.
func (d *Downloader) stateFetcher() {//首先试图往stateSyncStart通道来以获取信息。 而syncState这个函数会给stateSyncStart通道发送数据。
	for {
		select {
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {//从stateCh获取已经下载好的状态，然后把他投递到deliver通道上等待别人处理
	var (
	...
	for {
		...
		case next := <-d.stateSyncStart:// 另外一个stateSync申请运行。 我们退出。
			return next
		...
		case deliverReqCh <- deliverReq:// 发送已经下载好的数据给sync
		...
		case pack := <-d.stateCh:// 处理进入的数据包。 downloader接收到state的数据会发送到这个通道上面。
...
func (s *stateSync) loop() (err error) {
	...
	// Keep assigning new tasks until the sync completes or aborts        一直等到 sync完成或者被被终止
	for s.sched.Pending() > 0 {
		if err = s.commit(false); err != nil {// 把数据从缓存里面刷新到持久化存储里面。 这也就是命令行 --cache指定的大小。
			return err
		}
		s.assignTasks()// 指派任务
		...
		case req := <-s.deliver:// 接收到runStateSync方法投递过来的返回信息，注意 返回信息里面包含了成功请求的也包含了未成功请求的。
