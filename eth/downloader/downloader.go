...
type Downloader struct {
	...
	queue   *queue   // Scheduler for selecting the hashes to download  对象用来调度 区块头，交易，和收据的下载，以及下载完之后的组装
	peers   *peerSet // Set of active peers from which download can proceed 对端的集合
	stateDB ethdb.Database

	rttEstimate   uint64 // Round trip time to target for download requests  下载的往返时延
...
// Synchronise tries to sync up our local block chain with a remote peer, both   Synchronise试图和一个peer来同步，如果同步过程中遇
// adding various sanity checks as well as wrapping it with various log entries. 到一些错误，那么会删除掉Peer。然后会被重试。
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	if !atomic.CompareAndSwapInt32(&d.synchronising, 0, 1) {// 这个方法同时只能运行一个， 检查是否正在运行。
		return errBusy
	}
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	latest, err := d.fetchHeight(p)// 使用hash指来获取区块头，这个方法里面会访问网络
	if err != nil {
		return err
	}
	height := latest.Number.Uint64()

	origin, err := d.findAncestor(p, height)// findAncestor试图来获取大家共同的祖先，以便找到一个开始同步的点。
	if err != nil {
		return err
	}
	...
	pivot := uint64(0)
	if d.mode == FastSync {// 如果pivot这个点没有被锁定。
...
func (d *Downloader) spawnSync(fetchers []func() error) error {//给每个fetcher启动一个goroutine, 然后阻塞的等待fetcher出错。
...
// fetchHeaders keeps retrieving headers concurrently from the number             fetchHeaders不断的发送header请求，等待所有的返回。直到完成
// requested, until no more are returned, potentially throttling on the way. To   所有的header请求。 为了提高并发性，同时仍然能够防止恶意节点
// facilitate concurrency but still protect against malicious nodes sending bad   发送错误的header，我们使用我们正在同步的“origin”peer构造
// headers, we construct a header chain skeleton using the "origin" peer we are   一个头文件链骨架，并使用其他人填充缺失的header。 其他peer
// syncing with, and fill in the missing headers using anyone else. Headers from  的header只有在干净地映射到骨架上时才被接受。 如果没有人能够
// other peers are only accepted if they map cleanly to the skeleton. If no one   填充骨架 - 甚至origin peer也不能填充 - 它被认为是无效的，
// can fill in the skeleton - not even the origin peer - it's assumed invalid and 并且origin peer也被丢弃。
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64, pivot uint64) error {
	...
	for {
		...
		case packet := <-d.headerCh://网络上返回的header会投递到headerCh这个通道
			...
			if packet.Items() == 0 {// 如果没有更多的返回了。 那么告诉headerProcCh通道
			...
			if skeleton {// 如果是需要填充骨架，那么在这个方法里面填充好
				...
				from += uint64(proced)// proced代表已经处理完了多少个了。  所以只需要proced:后面的headers了
			}
			// Insert all the new headers and fetch the next batch
			if len(headers) > 0 {
				p.log.Trace("Scheduling new headers", "count", len(headers), "from", from)
				select {//投递到headerProcCh 然后继续循环。
...
// fetchBodies iteratively downloads the scheduled block bodies, taking any    fetchBodies持续的下载区块体，中间会使用到任何可以用的链接，
// available peers, reserving a chunk of blocks for each, waiting for delivery 为每一个链接保留一部分的区块体，等待区块被交付，并定期的检查是否超时。
...
// fetchParts iteratively downloads scheduled block parts, taking any available   fetchParts迭代地下载预定的块部分，取得任何可用的对等体，
// peers, reserving a chunk of fetch requests for each, waiting for delivery and  为每个部分预留大量的提取请求，等待交付并且还定期检查超时。
// also periodically checking for timeouts.
//
// As the scheduling/timeout logic mostly is the same for all downloaded data     由于调度/超时逻辑对于所有下载的数据类型大部分是相同的，所以
// types, this method is used by each for data gathering and is instrumented with 这个方法被用于不同的区块类型的下载，并且用各种回调函数来处
// various callbacks to handle the slight differences between processing them.    理它们之间的细微差别。
//
...
func (d *Downloader) fetchParts(errCancel error, deliveryCh chan dataPack, deliver func(dataPack) (int, error), wakeCh chan bool,
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {// 如果peer在之前被禁止而且没有在合适的时间deliver它的数据，那么忽略这个数据
		...
		case <-update:
			...
			// If there's nothing more to fetch, wait or terminate     // 任务全部完成。 那么退出 
			...
			for _, peer := range idles {
				...
				// have them.
				request, progress, err := reserve(peer, capacity(peer)) // 为某个peer请求分配任务。
				if err != nil {
					return err
				}
				if progress {
...
// processHeaders takes batches of retrieved headers from an input channel and  processHeaders批量的获取headers， 处理他们，并通过downloader
// keeps processing and scheduling them into the header chain and downloader's  的queue对象来调度他们。 直到错误发生或者处理结束。
// queue until the stream ends or a failure occurs.
func (d *Downloader) processHeaders(origin uint64, pivot uint64, td *big.Int) error {
	// Keep a count of uncertain headers to roll back
	rollback := []*types.Header{}//rollback 用来处理这种逻辑，如果某个点失败了。那么之前插入的2048个节点都要回滚。因为安全性达不到要求， 可以详细参考fast sync的文档。
		...
		case headers := <-d.headerProcCh:
			...
			for len(headers) > 0 {
				...
				if d.mode == FastSync || d.mode == LightSync {//如果是快速同步模式，或者是轻量级同步模式(只下载区块头)
					...
					frequency := fsHeaderCheckFrequency// 每隔多少个区块验证一次
				...
				if d.mode == FullSync || d.mode == FastSync { // 如果当前queue的容量容纳不下了。那么等待。
					...
					inserts := d.queue.Schedule(chunk, origin)// 调用Queue进行调度，下载body和receipts
			...
			for _, ch := range []chan bool{d.bodyWakeCh, d.receiptWakeCh} {// 给通道d.bodyWakeCh, d.receiptWakeCh发送消息，唤醒处理线程。
				select {
				case ch <- true:
				default:
				}
...
func (d *Downloader) processFullSyncContent() error {//从队列里面获取区块然后插入
	for {
...
func (d *Downloader) processFastSyncContent(latest *types.Header) error {
	...
	stateSync := d.syncState(latest.Root)// 启动状态同步
	...
	for {
		...
		results := d.queue.Results(oldPivot == nil) // Block if we're not monitoring pivot staleness // 等待队列输出处理完成的区块
		if len(results) == 0 {
		...
		if err := d.commitFastSyncData(beforeP, stateSync); err != nil {// 插入fast sync的数据
			return err
		}
		if P != nil {
			// If new pivot block found, cancel old state retrieval and restart
			if oldPivot != P {
				stateSync.Cancel()// 如果已经达到了 pivot point 那么等待状态同步完成，
		...
		if err := d.importBlockResults(afterP); err != nil {// 对于pivot point 之后的所有节点，都需要按照完全的处理。
			return err
		}
	}
}
