...
// partialMatches with a non-nil vector represents a section in which some sub-  partialMatches代表了部分匹配的结果。 比入有三个需要过滤的条件 addr1, 
// matchers have already found potential matches. Subsequent sub-matchers will   addr2, addr3 ，需要找到同时匹配这三个条件的数据。第一个匹配的结果会
// binary AND their matches with this vector. If vector is nil, it represents a  送给第二个，第二个把第一个的结果和自己的结果执行bit与操作，然后作为
// section to be processed by the first sub-matcher.                             匹配的结果送给第三个处理。
type partialMatches struct {
	section uint64
	bitset  []byte
}

// Retrieval represents a request for retrieval task assignments for a given       Retrieval代表了一次区块布隆过滤器索引的检索工作，这个对象被发送给eth/bloombits.go
// bit with the given number of fetch elements, or a response for such a request.  里面的startBloomHandlers来处理， 这个方法从数据库来加载布隆过滤器索引然
// It can also have the actual results set to be used as a delivery data struct.   后放在Bitsets里面返回。
//
...
func NewMatcher(sectionSize uint64, filters [][][]byte) *Matcher {
	...
	for _, filter := range filters {
		...
		// Accumulate the filter rules if no nil rule was within 在计算中 如果bloomBits中只要其中的一条能够找到。那么就认为整个成立
		if bloomBits != nil {
			m.filters = append(m.filters, bloomBits) //不同的bloomBits 需要同时成立，整个结果才能成立
		}
	}
	...
	for _, bloomIndexLists := range m.filters {
		for _, bloomIndexList := range bloomIndexLists {
			for _, bloomIndex := range bloomIndexList {
				m.addScheduler(bloomIndex)//对于所有可能出现的下标。 我们都生成一个scheduler来进行对应位置的布隆过滤数据的检索。
			}
		}
	}
	return m
...
func (m *Matcher) Start(ctx context.Context, begin, end uint64, results chan uint64) (*MatcherSession, error) {
	...
	// Initiate a new matching round    启动了一个session作为返回值，管理查找的生命周期
	...
	sink := m.run(begin, end, cap(results), session) //会建立起流程，返回了一个partialMatches类型的管道表示查询的部分结果
	...
	go func() {
		...
		for {
			...
			case res, ok := <-sink:       //找到返回结果 因为返回值是 section和 section中哪些区块可能有值的bitmap
				// New match result found //所以需要遍历这个bitmap，找到那些被置位的区块，把区块号返回回去。
				if !ok {
					return
				}
...
// 方法开始section indexer送到第一个子匹配器，并返回接收结果的接收器通道。
func (m *Matcher) run(begin, end uint64, buffer int, session *MatcherSession) chan *partialMatches {
	...
	session.pend.Add(1)
	go m.distributor(dist, session)// 启动distributor线程。
...
//subMatch创建一个子匹配器，用于过滤一组地址或主题，对这些主题进行bit位或操作，然后将上一个结果与当前过滤结果进行位与操作，如果结果不全位空，
//就把结果传递给下一个子匹配器。 每个地址/题目的匹配是通过获取属于该地址/题目的三个布隆过滤器位索引的给定部分以及将这些向量二进制AND并在一起来计算的。
func (m *Matcher) subMatch(source chan *partialMatches, dist chan *request, bloom []bloomIndexes, session *MatcherSession) chan *partialMatches {
	...
	for i, bits := range bloom {// i代表了第二维度的数量
		for j, bit := range bits { //j 代表了布隆过滤器的下标 肯定只有三个 取值(0-2048)
			...
			// 通过sectionSinks[i][j]来接收结果  // dist 是scheduler传递请求的通道。 
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-source:
				...
				for _, bloomSources := range sectionSources {
					for _, bitSource := range bloomSources {
						//传递给上面的所有的scheduler的输入通道。 申请对这些section 的指定bit进行查找。 结果会发送给sectionSinks[i][j]
						select {
						case <-session.quit:
							return
		...
		for {
			...
			case subres, ok := <-process:
				...
				for _, bloomSinks := range sectionSinks {
					var andVector []byte
					for _, bitSink := range bloomSinks {//对值进行与操作，就可以得到那些区块可能存在对应的值
						var data []byte
						select {
						case <-session.quit:
							return
...
// distributor receives requests from the schedulers and queues them into a set  接受来自scheduler的请求，并把他们放到一个set里面。 然后
// of pending requests, which are assigned to retrievers wanting to fulfil them. 把这些任务指派给retrievers来填充他们。
func (m *Matcher) distributor(dist chan *request, session *MatcherSession) {
	...
	for {
		...
		case fetcher := <-retrievers:
			// New retriever arrived, find the lowest section-ed bit to assign 如果新的retrievers进来了， 那么我们查看是否有任务没有指派
		...
		case fetcher := <-m.counters:
			// New task count request arrives, return number of items 来了新的请求，访问request的指定bit的数量。
			fetcher <- uint(len(requests[<-fetcher]))

		case fetcher := <-m.retrievals:
			// New fetcher waiting for tasks to retrieve, assign  // 有人来领取任务。
			task := <-fetcher
			...
			if len(requests[task.Bit]) > 0 {// 如果还有任务没有分派完。 尝试分配给其他人。
				assign(task.Bit)
			}

		case result := <-m.deliveries:
			// New retrieval task response from fetcher, split out missing sections and
			// deliver complete ones        // 收到了任务的结果。
			var (
			...
			m.schedulers[result.Bit].deliver(sections, bitsets)// 投递结果
			allocs--

			// Reschedule missing sections and allocate bit if newly available //如果有缺失， 那么重新生成新的任务。
			if len(missing) > 0 {
...
func (s *MatcherSession) AllocateRetrieval() (uint, bool) {//任务领取了一个任务。 会返回指定的bit的检索任务。
...
func (s *MatcherSession) AllocateSections(bit uint, count int) []uint64 {//领取指定bit的section查询任务
...
func (s *MatcherSession) DeliverSections(bit uint, sections []uint64, bitsets [][]byte) {//把结果投递给deliveries 通道
	select {
	case <-s.kill:
		return
