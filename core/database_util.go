...
package core
//参考：http://www.ethchinese.com/?p=1827
import (
	"bytes"
	"encoding/binary"
	"encoding/json"
...
//hash就是该Block(或Header)对象的RLP哈希值，在代码中也被称为canonical hash；num是Number的uint64类型,大端(big endian)整型数。
//num用来确定Block在整个区块链中所处的位置，hash用来辨识惟一的Block/Header对象。
//除此之外，底层数据库里还分别存有当前最新Block和Header的canonical hash：
//key 	        value
//“LastHeader” 	hash
//“LastBlock” 	hash
//“LastFast” 	hash
//这里“LastFast”所存储的是在一种特别的同步方式FastSync下，最新Block的canonical hash。FastSync相比于FullSync，可以仅仅同步Header而不考虑Body
//**以BlockChain为例，通过”LastBlock”为key从数据库中获取最新的Block之后，用num逐一遍历，得到目标Block的num后，用’h’+num+’n’作key，
//**就可以从数据库中获取目标canonical hash。
var (
...
type TxLookupEntry struct { //交易的Meta信息结构
...
func GetTransaction(db DatabaseReader, hash common.Hash) (*types.Transaction, common.Hash, uint64, uint64) { //从数据库中读取交易信息
	// Retrieve the lookup metadata and resolve the transaction from the body
	// 通过交易hash从数据库中获取交易的meta信息，包括交易所在块的hash，块号和第几笔交易
	blockHash, blockNumber, txIndex := GetTxLookupEntry(db, hash)
	//使用块号和块hash获取从数据库中读取块的信息
	if blockHash != (common.Hash{}) {
		...
		return body.Transactions[txIndex], blockHash, blockNumber, txIndex //根据第几笔交易从块上获取交易的具体信息
	}
	// Old transaction representation, load the transaction and it's metadata separately
	// 以前的交易存储则是需要存储交易的具体信息和meta信息
...
//区块头的存储格式为：headerPrefix + num (uint64 big endian) + hash -> rlpEncode(header)
//其中key由区块头前缀、区块号（uint64大端格式）、区块hash构成，value是区块头的RLP编码。
// WriteHeader serializes a block header into the database.
func WriteHeader(db ethdb.Putter, header *types.Header) error {  //区块头的存储
	data, err := rlp.EncodeToBytes(header)       //区块头的RLP编码
	if err != nil {
		return err
	}
	hash := header.Hash().Bytes()                //区块头hash
	num := header.Number.Uint64()                //区块号
	encNum := encodeBlockNumber(num)             //区块号（uint64大端格式）
	key := append(blockHashPrefix, hash...)      //组装key
	if err := db.Put(key, encNum); err != nil {  //向数据库中存储一条'区块hash->区块号'的记录，最后将区块头的RLP编码写到数据库中
...
//区块体的存储格式为：bodyPrefix + num (uint64 big endian) + hash -> rlpEncode(block body)
...
func WriteBody(db ethdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
	...
	return WriteBodyRLP(db, hash, number, data)   //调用WriteBodyRLP将区块体的RLP编码写到数据库中
}

// WriteBodyRLP writes a serialized body of a block into the database.
func WriteBodyRLP(db ethdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
	key := append(append(bodyPrefix, encodeBlockNumber(number)...), hash.Bytes()...)
	if err := db.Put(key, rlp); err != nil {      //WriteBodyRLP根据上面的规则组装key，然后向数据库中写入一条记录
...
// WriteBlock serializes a block into the database, header and body separately.  将一个区块的信息写入数据库
...
func WriteTxLookupEntries(db ethdb.Putter, block *types.Block) error { //将一笔交易写入数据库中
	// Iterate over each transaction and encode its metadata
	for i, tx := range block.Transactions() { //遍历块上的所有交易，并构造交易的meta信息，进行RLP编码。
		entry := TxLookupEntry{
			BlockHash:  block.Hash(),
			BlockIndex: block.NumberU64(),
			Index:      uint64(i),
		}
		data, err := rlp.EncodeToBytes(entry)  //交易的RLP编码
		if err != nil {
			return err
		}
		if err := db.Put(append(lookupPrefix, tx.Hash().Bytes()...), data); err != nil {
			return err
		}//以交易的hash为key、交易的RLP编码为value存储
	}
	return nil
}
