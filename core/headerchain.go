...
type HeaderChain struct {
	...
	currentHeader     atomic.Value // Current head of the header chain (may be above the block chain!)       当前区块头
...
func NewHeaderChain(chainDb ethdb.Database, config *params.ChainConfig, engine consensus.Engine, procInterrupt func() bool) (*HeaderChain, error) {
	...
	// Seed a fast but crypto originating random generator        产生随机数
	...
	hc.genesisHeader = hc.GetHeaderByNumber(0)                 //判断创世区块头是否为空
	...
	if head := GetHeadBlockHash(chainDb); head != (common.Hash{}) {
		if chead := hc.GetHeaderByHash(head); chead != nil {
			hc.currentHeader.Store(chead)                      //设置当前区块头的哈希值
		}
	}
