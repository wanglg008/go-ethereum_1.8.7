...
func (s *dialstate) newTasks(nRunning int, peers map[discover.NodeID]*Peer, now time.Time) []task {
	...
	// Create dials for static nodes if they are not connected. //查看所有的静态类型。如果可以那么也创建链接。
	...
	// dynamic dials.
	randomCandidates := needDynDials / 2        //否则使用1/2的随机节点创建链接。
...
func (s *dialstate) taskDone(t task, now time.Time) {
	switch t := t.(type) {  //查看task的类型。
	case *dialTask:         //如果是链接任务，那么增加到hist里面。 并从正在链接的队列删除。
		s.hist.add(t.dest.ID, now.Add(dialHistoryExpiration))
		delete(s.dialing, t.dest.ID)
	case *discoverTask:     //如果是查询任务。 把查询的记过放在lookupBuf里面。
