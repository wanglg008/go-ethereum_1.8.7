...
func New(config Config) (*Console, error) {   //创建一个console
	// Handle unset config values gracefully  首先对Config做一些默认设置，接着调用其jsre.New函数，在借这个调用init方法初始化相关RPC API。
...
func (c *Console) Interactive() {
	var (
		...
		//首先创建一个scheduler通道，用于进程间的通信。
		scheduler = make(chan string) // Channel to send the next prompt on and receive the input
	)
	// Start a goroutine to listen for promt requests and send back inputs
	// 接着使用goroutine的方式创建一个线程来监听用户在控制台的输入。
	go func() {
		for { 						  //接着一个无线for循环来处理从scheduler传递过来用户的输入信息。
			// Read the next user input   首先通过从scheduler通道读取数据line，然后做一些数据合法性验证。
	...
	//接着调用console的Evaluate方法将用户输入的命令交给前面的jsre处理（最终调用Do方法向evalQueue通道发送数据），最终前面初始化console时初始化的
	//jsre用于接收用户输入命令的线程接收到数据，然后调用VM来处理，并将结果返回在控制台显示。
	for {
